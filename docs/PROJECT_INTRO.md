...
当前版本已完成 Codex 真实自动闭环验证，并接通 OpenCode、Claude Code 的非交互 Adapter；后两者仍应在各自凭证和模型环境中完成首次启动验证。Runner 会隔离单项目故障、持续读取 Agent 输出并报告真实 launchd 健康状态；遇到不确定、权限、密钥或高风险操作时，仍应进入 `BLOCKED` 并交给用户。
