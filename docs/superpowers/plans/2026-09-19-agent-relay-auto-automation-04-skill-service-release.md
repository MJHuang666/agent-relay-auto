# Agent Relay Auto 自动闭环阶段 4：Skill、launchd 与发行 Implementation Plan
...
**Architecture:** Skill 目录是源码真源；安装器把带版本的只读运行时复制到 `~/.local/share/agent-relay-auto/versions/<version>/`，用原子 current 指针切换。launchd 只引用稳定运行目录。项目内只提交策略与运行快照，本机和用户级配置分别留在忽略目录与 `~/.config/agent-relay-auto/`。
...
**Files:**
- Create: `shared/.agents/skills/agent-relay-auto/scripts/setup_runner.py`
- Create: `shared/.agents/skills/agent-relay-auto/scripts/runnerctl.py`
- Create: `shared/.agents/skills/agent-relay-auto/assets/launchd/com.agent-relay-auto.runner.plist.in`
...
```bash
git add shared/.agents/skills/agent-relay-auto/scripts shared/.agents/skills/agent-relay-auto/assets/launchd tests/test_runner_installer.py tests/test_runnerctl.py
...
**Files:**
- Create: `shared/.agents/skills/agent-relay-auto/scripts/configure_runtime.py`
...
4. [ ] 项目策略写入可提交文件；CLI 路径/session/主机能力写 `.agent-relay-auto/local.yaml`；共享运行 profile 写用户配置。任何 secret value 均拒绝写入。
5. [ ] 初始化把 `.agent-relay-auto/` 加入 `.gitignore`，保存活动任务的 runtime snapshot，并注册项目；Runner 未获安装确认时保持 manual mode。
...
- Create: `shared/.agents/skills/agent-relay-auto/references/runner.md`
- Modify: corresponding Skill mirror files under `assets/project-template/shared/.agents/skills/agent-relay-auto/`
- Modify: `README.md`
...
7. [ ] 运行契约测试和 `rg -n '\$project-role-workflow' README* docs/AGENT_RELAY_AUTO_USAGE*`，公开文档不得恢复旧品牌命令。
8. [ ] 提交：

```bash
git add shared/.agents/skills/agent-relay-auto README.md README.zh-CN.md docs tests/test_agent_relay_contract.py
...
2. [ ] 扩展 clone recovery，证明提交的任务/队列/快照在新 clone 可恢复，而 `.agent-relay-auto/runs` 与原生 session 不进入 Git。
...
6. [ ] 更新版本号为实施时确定的下一个语义版本；生成 `agent-relay-auto-skill-pack-v<version>.zip` 和同名 `.sha256`，压缩包不得包含 `.git`、`.agent-relay-auto`、日志或凭证。
7. [ ] 运行：
