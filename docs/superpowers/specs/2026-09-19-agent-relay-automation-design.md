...
本设计在 Agent Relay 现有 Planner、Implementer、Reviewer 文件协作协议之上增加本地 Runner，使一个任务在需求明确后能够自动完成规划、实施、审查、返修、重新规划、交付报告和结束状态转换。
...
Runner 源码、适配器、安装脚本和 launchd 模板随 `agent-relay` Skill 分发：
...
~/Library/LaunchAgents/com.agent-relay.runner.plist
```
...
`.agent-relay/` 必须加入目标项目 `.gitignore`。运行元数据至少包含 task、role、participant、Agent、model、PID、进程启动时间、run ID、起始 revision、状态、退出码和最新 heartbeat。
...
项目本机，忽略提交：
.agent-relay/local.yaml
.agent-relay/runs/

用户级，不属于仓库：
~/.config/agent-relay/runtime-profiles.yaml
~/.config/agent-relay/projects.yaml
```
