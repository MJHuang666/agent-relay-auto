...
3. 把共享 Skill（包括 `scripts/workflow_state.py`）安装到项目根 `.agents/skills/agent-relay-auto/`，把共享状态模板安装到 `docs/agent/`。如果宿主策略禁止写入 `.agents/`，继续使用已加载的全局 Skill 和 `docs/agent/protocol.md`，并报告项目级 Skill 为 pending；不要把它当作安装失败。
...
如果用户要求安装为个人或全局 Skill，则把 `shared/.agents/skills/agent-relay-auto/` 安装到当前工具的个人 Skills 目录。安装完成后，在空白仓库中调用 `$agent-relay-auto 初始化当前仓库` 或 `$agent-relay-auto initialize this repository` 即可自举项目文件。
