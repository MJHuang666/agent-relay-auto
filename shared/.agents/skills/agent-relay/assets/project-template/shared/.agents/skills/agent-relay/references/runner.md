...
Runtime source is copied to `~/.local/share/agent-relay/versions/<version>/`; launchd points at the stable `current` link. User configuration is under `~/.config/agent-relay/`, while project-local `.agent-relay/` contains ignored process logs, run metadata, heartbeats, and native session references.
