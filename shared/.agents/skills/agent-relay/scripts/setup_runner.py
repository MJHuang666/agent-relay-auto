#!/usr/bin/env python3
"""Install a versioned Agent Relay Runner copy and launchd plist."""
...
<key>Label</key><string>com.agent-relay.runner</string>
...
'''
    ...
    def install(self, dry_run: bool = False) -> dict[str, object]:
        actions = [
            ...
            f"write {self.paths.launch_agents / 'com.agent-relay.runner.plist'}",
        ]
        if dry_run:
        ...
        (self.paths.launch_agents / "com.agent-relay.runner.plist").write_text(self._plist(), encoding="utf-8")
    ...
    def uninstall(self, keep_config: bool = True, dry_run: bool = False) -> dict[str, object]:
        actions = [f"remove {self.paths.launch_agents / 'com.agent-relay.runner.plist'}", f"remove {self.paths.data_root}"]
        if not keep_config:
            actions.append(f"remove {self.paths.config_root}")
        if dry_run:
            return {"status": "dry-run", "actions": actions}
        plist = self.paths.launch_agents / "com.agent-relay.runner.plist"
        if plist.exists():
            plist.unlink()
...
def main(argv=None) -> int:
    ...
    home = Path.home()
    paths = InstallPaths(home / ".local/share/agent-relay", home / ".config/agent-relay", home / "Library/LaunchAgents")
